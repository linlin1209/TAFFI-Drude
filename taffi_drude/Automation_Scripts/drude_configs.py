...
def main(argv):
    ...
    if args.write_bash:
       ...
       jsonfile = '{}/all.json'.format(args.nonpolar_dir)
       if os.path.isfile(jsonfile) is False:
         print("ERROR: {} not found, Exiting...".format(jsonfile))
         quit()
       status = read_alljson(jsonfile)
    ...
    if os.path.isdir('{}/{}'.format(current_dir,args.folder)) is False:
      os.makedirs('{}/{}'.format(current_dir,args.folder))
...
def polar_orca(calc_dir,Geometry,Elements):
   ...
   random_factor = 0.0
   if os.path.isfile('polar.out') and check_orca_complete('polar.out') is False:
      os.remove('polar.out')
   else:
      return
   with open("polar.in",'w') as f:
      ...
      #f.write('! {} {} TIGHTSCF {} CHELPG PAL{} KeepDens\n\n'.format(c['functional'],c['basis'],D3_option,c["charges_qc_ppn"]))
      f.write("! {} {} TIGHTSCF {} CHELPG PMODEL PAL{}\n".format(c['functional'],c['basis'],D3_option,c["charges_qc_ppn"]))
...
def calc_CHELPG(calc_folder,xyz,total_pc,pc_layer1,pc_layer2,pc_layer3,geo,element,Qmag,tag):
    ...
    for i in range(0,total_pc):
         ...
         # if an out file is present here that means the job fails, need to remove out file so orca_submit can submit it again
         if os.path.isfile('charge.out'):
            os.remove('charge.out')
         if os.path.isfile('{}_charges.gbw'.format(i)):
            os.remove('{}_charges.gbw'.format(i))
