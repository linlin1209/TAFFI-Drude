...
import sys,os,argparse,subprocess,shutil,time,glob,getpass,json,codecs
import numpy as np
...
def fit_Drude(c,tag):
    ...
    for g in gens: 
      ...
      for inchi in mc_keys:
         ...
         # write out drude_Q.db for each inchikey
         ...
         # VDW reparametrization
         VDW_re(run_dir,args,tag,inchi,g,xyz)

         # merge inchikey's new charge with current generation drude db
         option['new_params'] = '{}/drude_Q.db'.format(run_dir)
         merge_FF_drude.fun(option)

         # merge inchikey's new vdw with current generation drude db
...
def VDW_re(run_dir,args,tag,inchikey,current_gen,xyz):
    ...
    elif os.path.isfile('{}/{}/lin.prm'.format(vdwfolder,FFname)) is False:
      print('ERROR: {}/{}/lin.prm not found, something went wrong for db_to_charmm, Exiting...'.format(vdwfolder,FFname))
      quit()
